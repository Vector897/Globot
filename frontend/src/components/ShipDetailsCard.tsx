            ...
            >
                ...
                <div className="relative h-24 bg-gradient-to-r from-[#1a2332] to-[#0f1621] p-4 flex flex-col justify-end border-b border-[#1a2332] cursor-move group">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                        <span className={`px-1.5 py-0.5 rounded-xs text-[10px] font-bold uppercase tracking-wider border ${ship.status === 'At Risk' ? 'bg-red-500/20 text-red-500 border-red-500/30' :
                            ship.status === 'Diverting' ? 'bg-orange-500/20 text-orange-500 border-orange-500/30' :
                                'bg-emerald-500/20 text-emerald-500 border-emerald-500/30'
                            }`}>
                            {ship.status}
                        </span>
                        <span className="text-[10px] text-white/40 font-mono tracking-wider">{(ship.type || 'vessel').toUpperCase()}</span>
                    </div>
